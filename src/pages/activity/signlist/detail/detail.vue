<template>
	<view v-if="signDetail && signDetail.signId" class="p20">
		<view class="header">
			<van-divider contentPosition="center" customStyle="color: #1989fa; border-color: #1989fa; font-size: 1rem;">
				加入活动口令
			</van-divider>
			...
			<view class="status p20">
				...
				<!-- 倒计时 -->
				<van-count-down format="mm 分 ss 秒 SSS毫秒" v-if="signDetail.status===SignStatus.ing" millisecond :time="remainTime" />
			</view>
			<view v-show="SignStatus.ing === signDetail.status">
				<view class="qrcode p20">
					<tki-qrcode :size="400" ref="qrcode" :val="qcValue" background="#fff" foreground="#000"
						pdground="#000" :onval="true" :showLoading="false" />
				</view>
				<!-- TODO:口令待确定 -->
		...
		<view class="control p20">
			<van-button block round type="primary" v-show="SignStatus.over === signDetail.status" @click="handleStart">
				开始
			</van-button>
			<van-button block round type="danger" v-show="SignStatus.ing === signDetail.status" @click="handleOver">
				立即结束
			</van-button>
			<van-button block round type="info" v-show="SignStatus.ing === signDetail.status" @click="handlePause">
				暂停
			</van-button>
			<van-button block round type="info" v-show="SignStatus.pause === signDetail.status" @click="handleStart">
				继续
			</van-button>
			<view v-show="SignStatus.ing !== signDetail.status">
				<van-divider contentPosition="center"
		...
		<view>
			...
			<view>
				<van-empty v-if="records.length === 0" image="search" description="没有数据喔" />
				<van-cell-group>
					<van-cell is-link v-for="(d, idx) in records" :key="idx" @click="handleCheckInfo(d)">
						<view slot="title">
				          ...
				          RecordStatusText[d.status]
								...
								<text class="rank"
									v-show="d.status === RecordStatus.success || d.status === RecordStatus.late">{{ d.rank }}</text>
							</view>
						</view>
					</van-cell>
				</van-cell-group>
		...
		computed: {
			...
			successPer(){
				...
				const success = this.signRecords.filter(s=>{
					return s.status===this.RecordStatus.success || s.status===this.RecordStatus.late
				}).length
				return ((success/sum)*100).toFixed(2)
			},
			activityPwd() {
				return this?.signDetail?.activityId && this.getActivityById(this.signDetail.activityId).pwd
			},
			signDetail() {
