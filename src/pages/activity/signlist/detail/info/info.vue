	...
	<view class="p20">
		<view v-if="record.name">
			...
			<van-cell-group>
				...
				<van-cell v-show="record.rank > 0" title="签到排名" :value="record.rank" />
				<van-cell title="状态">
			...
			<picker class="p20" mode="selector" :range="statusRange" range-key="text" @change="handleChangeStauts">
				<van-button round type="info" block>更改状态</van-button>
			</picker>
			<view class="p20">
				<van-button @click="changeTips" :disabled="!tips" round type="info" block>更改状态</van-button>
			</view>
			<van-cell-group>
				<van-field autosize placeholder="在此处输入新的批注内容" label="成员名称" :value="tips" @change="(e)=>{tips=e.detail}" />
			</van-cell-group>
		</view>
	</view>
</template>
